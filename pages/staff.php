...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    foreach (rex_clang::getAll() as $rex_clang) {
        if (false === $staff) {
            $staff = new TobiasKrais\D2UStaff\Staff($staff_id, $rex_clang->getId());
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $staff = new TobiasKrais\D2UStaff\Staff($staff_id, (int) rex_config::get('d2u_helper', 'default_lang'));
    ...
    <form action="<?= BackendHelper::getCurrentBackendPage([], ['message', 'message_type']) ?>" method="post">
		...
		<div class="panel panel-edit">
			...
			<div class="panel-body">
				...
				<?php
                    foreach (rex_clang::getAll() as $rex_clang) {
                        $staff = new TobiasKrais\D2UStaff\Staff($entry_id, $rex_clang->getId());
...
if ('' === $func) {
    ...
    if (rex::getUser() instanceof rex_user && (rex::getUser()->isAdmin() || rex::getUser()->hasPerm('d2u_staff[edit_data]'))) {
        $list->addColumn(rex_i18n::msg('status_online'), '<a class="rex-###online_status###" href="' . BackendHelper::getCurrentBackendPage(['func' => 'changestatus', 'entry_id' => '###staff_id###'], [], true) . '"><i class="rex-icon rex-icon-###online_status###"></i> ###online_status###</a>');
        $list->setColumnLayout(rex_i18n::msg('status_online'), ['', '<td class="rex-table-action">###VALUE###</td>']);

        $list->addColumn(rex_i18n::msg('delete_module'), '<i class="rex-icon rex-icon-delete"></i> ' . rex_i18n::msg('delete'));
        $list->setColumnLayout(rex_i18n::msg('delete_module'), ['', '<td class="rex-table-action">###VALUE###</td>']);
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###staff_id###'] + $csrfToken->getUrlParams());
